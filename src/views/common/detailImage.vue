    ...
    <div class="outBox" ref="outBox" @click="closeOutBox($event)">
      <div class="box" ref="box">
        <div class="item">
          <div class="content">
            <div class="image">
              <el-carousel style="border-radius: 2vw">
                <el-carousel-item v-for="(item, index) in detailPinia.images" :key="index">
                  <el-image class="img" fit="cover" :src="item" :preview-src-list="detailPinia.images"
                    :preview-teleported="true" :initial-index="index" :hide-on-click-modal="true" />
                </el-carousel-item>
              </el-carousel>
            </div>
...
const userItem = user();
const detailPinia = ref(JSON.parse(localStorage.getItem("graphicDetail") as string));
...
function comment(id: number) {
  const res = getComment({ id: id, type: "graphic" } as any);
  res.then((r: any) => {
    if (r.length > 0) {
      commentData.value = r;
...
function reply(ids: number[], index: number) {
  const data = { ids: ids, index: index, type: "graphic" };
...
const closeOutBox = async (event: any) => {
  function back() {
    return new Promise<void>((resolve) => {
      const { w, x, y, h } = JSON.parse(localStorage.getItem("graphicDetail") as string)
